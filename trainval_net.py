    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            for _ in range(2): # to follow caffe work
                ...
                im_info.data.resize_(data[4].size()).copy_(data[4])
                labels = torch.squeeze(labels).view(im_data.size(0), labels.size(-1))
                rois, midn_loss, oc1, oc2, oc3, cls_prob, rpn_loss_cls, rpn_loss_bbox= OICR(im_data, im_rois, im_info, labels, num_boxes)
                oicr_loss = oc1 + oc2 + oc3
                rois_label= cls_prob
                loss_total = midn_loss + oicr_loss + rpn_loss_bbox + rpn_loss_cls
                rpn_loss = rpn_loss_cls + rpn_loss_bbox
                midn_loss = midn_loss.mean()
                oicr_loss = oicr_loss.mean()
                rpn_loss_cls = rpn_loss_cls.mean()
                rpn_loss_bbox = rpn_loss_bbox.mean()
                oc1 = oc1.mean()
                oc2 = oc2.mean()
                oc3 = oc3.mean()
                loss = midn_loss + oicr_loss + rpn_loss_bbox + rpn_loss_cls
                loss_temp += loss.item()
                ...
                rpn_loss_bbox_temp += rpn_loss_bbox.item()
                rpn_loss_cls_temp += rpn_loss_cls.item()
                # backward
            ...
            if total_step % args.disp_interval == 0:
                ...
                if step > 0:
                    ...
                    rpn_loss_bbox_temp /= ((args.disp_interval + 1))
                    rpn_loss_cls_temp /= ((args.disp_interval + 1))
                ...
                logger.record_tabular('rpn_cls_loss', rpn_loss_cls_temp)
                logger.record_tabular('rpn_bbox_loss', rpn_loss_bbox_temp)
